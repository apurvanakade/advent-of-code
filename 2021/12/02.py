...
def DFS(current_node, path, num_paths, is_vertex_repeated):
  ...
  for neighbor in neighbors:
    if neighbor == start_node:
      ...
      new_paths += DFS(neighbor, path, num_paths, is_vertex_repeated)
      path.pop()
    elif not is_vertex_repeated:
      path.append(neighbor)
      new_paths += DFS(neighbor, path, num_paths, True)
      path.pop()

  return num_paths + new_paths

print(DFS (start_node, [start_node], 0, False))
